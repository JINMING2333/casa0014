...
void loop() {
  ...
  if (irValue < 50000) { // 手指离开
        temperature = 0.0; // 清零体温
        heartRate = 0;     // 清零心率
        updateOLED("No Finger", 0); // 更新 OLED 显示
    } else {
        if (mode == 0) {
          heartbeat(); 
          updateOLED("Heart Rate", heartRate); 
        } else if (mode == 1) {
          monitorTemperature();
          updateOLED("Temperature", temperature); 
        }
    }
}
...
void detectClick() {
  ...
  if (irValue > 100000 && !tapState && currentTime - lastTapTime > debounceTime) {
    tapState = true; 
