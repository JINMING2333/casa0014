...
// 定义全局变量用于渐变控制
int warmRed = 255;     // 红色分量
int warmGreen = 100;   // 绿色分量起点
int warmBlue = 0;      // 蓝色分量保持较低
bool greenIncreasing = true; // 绿色通道递增标志
...
void setup() {
  ...
  particleSensor.setSampleRate(100);         // 提高采样率到 100Hz
  particleSensor.setFIFOAverage(4);          // 平均 4 个样本
  particleSensor.enableFIFORollover();       // 启用 FIFO 回绕避免数据丢失
...
void heartbeat(){
  ...
  smoothedIR = (smoothedIR * 0.95) + (irValue * 0.05); // 低通滤波
  long deltaIR = smoothedIR - previousIR;            // 计算平滑后的差值
